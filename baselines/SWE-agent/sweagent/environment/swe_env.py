...
class EnvironmentConfig(BaseModel):
    ...
    deployment: DeploymentConfig = Field(
        default_factory=lambda: DockerDeploymentConfig(image="python:3.12", python_standalone_dir="/root"),
...
class SWEEnv:
    ...
    def start(self) -> None:
        ...
        for command in self._post_startup_commands:
            self.communicate(command, check="warn", timeout=self.post_startup_command_timeout)
    ...
    def _copy_repo(self) -> None:
        ...
        if self.repo.repo_name in folders:
            self.communicate(input=f"rm -rf /{self.repo.repo_name}", check="raise")
            folders = self.communicate(input="ls", check="raise").split()
            assert self.repo.repo_name not in folders, f"repo {self.repo.repo_name} is not removed"
    ...
    def reset_to_state(self, state: dict) -> None:
        """
        Resets the environment to a specific state defined by a diff.

        Args:
            state: A dictionary containing the `diff` of the state to restore.
                   The diff should be relative to the base commit of the repo.
        """
        self.logger.info("Resetting environment to a specific state.")

        # 1. Reset repo to base commit
        self._reset_repository_hard()

        diff = state.get("diff")

        if not diff or not isinstance(diff, str):
            self.logger.info("No valid diff provided in the state. Environment is in a clean state.")
            return
        
        if self.repo is None:
            self.logger.warning("No repository configured, cannot apply diff.")
            return

        # 2. Apply the diff
        # The CWD is the repository root after _reset_repository()
        patch_file = "state.patch"
        repo_path = f"/{self.repo.repo_name}"
        patch_path = f"{repo_path}/{patch_file}"
        
        # Normalize line endings to LF
        if not diff.endswith("\n"):
            diff += "\n"
        self.write_file(patch_path, diff)

        # Use --unidiff-zero to allow applying patches with no context lines
        # Use --inaccurate-eof to allow applying patches that don't have a newline at the end of the file
        apply_command = f"git apply --unidiff-zero --inaccurate-eof {patch_file}"
        self.communicate(apply_command, check="raise", error_msg=f"Failed to apply diff from patch file {patch_path}")
        # 3. Clean up the patch file
        rm_command = f"rm {patch_file}"
        self.communicate(rm_command, check="raise", error_msg=f"Failed to remove patch file {patch_path}")

        self.logger.info("Successfully applied diff and reset to state.")
    ...
    def _reset_repository_hard(self) -> None:
        """Clean repository of any modifications + Checkout base commit using reset --hard"""
        if self.repo is not None:
            self.logger.debug("Hard resetting repository %s to commit %s", self.repo.repo_name, self.repo.base_commit)
            # NOTE: `checkout` is not forceful enough and can leave modified files behind, causing `git apply`
            # to fail. We are switching to `reset --hard` which is the correct way to ensure a clean state.
            startup_commands = [
                f"cd /{self.repo.repo_name}",
                "export ROOT=$(pwd -P)",
                "git status",
    ...
    def set_env_variables(self, env_variables: dict[str, str]) -> None:
        ...
        self.communicate(command, check="raise", timeout=120)
