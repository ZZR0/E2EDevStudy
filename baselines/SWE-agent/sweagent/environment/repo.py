...
class LocalRepoConfig(BaseModel):
    ...
    def copy(self, deployment: AbstractDeployment):
        ...
        # r = asyncio.run(deployment.runtime.execute(Command(command=f"chown -R root:root {self.repo_name}", shell=True))) # 原始的代码
        r = asyncio.run(deployment.runtime.execute(Command(command=f"chown -R root:root /{self.repo_name}", shell=True))) # 修改后的代码
        if r.exit_code != 0:
