...
class DefaultAdvAgentConfig(BaseModel):
    """This configuration object specifies the behavior of an agent."""
    ...
    type: Literal["default_adv"] = "default_adv"
...
class DevTestAgentConfig(BaseModel):
    name: str = "dev_test"
    dev: DefaultAgentConfig
    test: DefaultAgentConfig
    type: Literal["dev_test"] = "dev_test"
    model_config = ConfigDict(extra="forbid")

class DesignDevTestAgentConfig(BaseModel):
    name: str = "design_dev_test"
    design: DefaultAgentConfig
    dev: DefaultAgentConfig
    test: DefaultAgentConfig
    type: Literal["design_dev_test"] = "design_dev_test"
    model_config = ConfigDict(extra="forbid")

AgentConfig = Annotated[DefaultAgentConfig | RetryAgentConfig | DevTestAgentConfig | DefaultAdvAgentConfig | DesignDevTestAgentConfig, Field(union_mode="left_to_right")]
...
EXIT_GIVEUP_TOKEN = "###SWE-AGENT-EXIT-GIVEUP###"
...
def get_agent_from_config(config: AgentConfig) -> AbstractAgent:
    ...
    elif config.type == "default_adv":
        from .agents_adv import AdvDefaultAgent
        return AdvDefaultAgent.from_config(config)
    elif config.type == "dev_test":
        from .agents_adv import DevTestAgent
        return DevTestAgent.from_config(config)
    elif config.type == "design_dev_test":
        from .agents_adv import DesignDevTestAgent
        return DesignDevTestAgent.from_config(config)
    else:
...
class DefaultAgent(AbstractAgent):
    ...
    def add_system_message_to_history(self) -> None:
        ...
        assert self._problem_statement is not None
        system_msg = Template(self.templates.system_template).render(**self._get_format_dict(**self.tools.get_state(self._env)))
    ...
    def handle_action(self, step: StepOutput) -> StepOutput:
        ...
        if RETRY_WITH_OUTPUT_TOKEN in step.observation:
            ...
            raise _ExitForfeit()
        elif EXIT_GIVEUP_TOKEN in step.observation:
            raise _ExitGiveup()
    ...
    def check_stuck_in_loop(self) -> bool:
        """Check if the agent is stuck in a loop.
        
        Returns:
            bool: True if the agent is stuck in a loop, False otherwise
        """
        model_history = [item for item in self.history if item["role"] == "assistant"]
        if len(model_history) < 5:
            return False
        if model_history[-1]['action'] == model_history[-2]['action'] \
            and model_history[-1]['action'] == model_history[-3]['action'] \
            and model_history[-1]['action'] == model_history[-4]['action'] \
            and model_history[-1]['action'] == model_history[-5]['action']:
            return True
        return False
    ...
    def forward_with_handling(self, history: list[dict[str, str]]) -> StepOutput:
        ...
        # if self.check_stuck_in_loop():
        #     self.logger.warning("Stuck in a loop, exiting")
        #     return handle_error_with_autosubmission(
        #         "exit_loop",
        #         "Stuck in a loop, exiting",
        #     )
        ...
        while n_format_fails < self.max_requeries:
            ...
            # Errors that cause exit
                ...
                self.logger.info("Exiting due to giveup")
                return handle_error_with_autosubmission(
                    "exit_giveup",
                    "Exiting due to giveup",
                )
