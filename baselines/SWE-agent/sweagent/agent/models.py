...
class GenericAPIModelConfig(PydanticBaseModel):
    ...
    thread_key_group_size: int = 5
    """If `choose_api_key_by_thread` is True, this defines the size of the key group
    assigned to each thread. A key is then randomly selected from the assigned group.
    """
    ...
    def choose_api_key(self) -> str | None:
        ...
        if k > num_keys:
            get_logger("config", emoji="🔧").debug(
                f"Group size {k} is larger than number of keys {num_keys}. "
                f"Each thread will pick from all {num_keys} keys."
            )

        # Create non-overlapping groups of keys.
        # If k=1, each thread gets its own key, cycling through.
        # If k >= num_keys, all threads share all keys.
        # Otherwise, keys are partitioned into non-overlapping groups of size k
        # (the last group may be smaller), and threads are assigned to groups.
        num_groups = (num_keys + k - 1) // k
        group_idx = thread_idx % num_groups

        start_of_group = group_idx * k
        end_of_group = min(start_of_group + k, num_keys)
        key_group = api_keys[start_of_group:end_of_group]

        chosen_key = random.choice(key_group)
        key_idx = api_keys.index(chosen_key)

        get_logger("config", emoji="🔧").debug(
            f"Choosing API key {key_idx} for thread {thread_name} (idx {thread_idx}) from key group {group_idx} "
            f"(indices {start_of_group}-{end_of_group - 1})"
        )
        return chosen_key
...
class LiteLLMModel(AbstractModel):
    ...
    def _single_query(
        ...
        try:
            stream = "gemini-2.5-pro" in self.config.name
