...
from harness.utils.utils import clone_repo, EvalReport, load_file, save_file
from harness.utils.dockerfiles import DOCKERFILE
from harness.utils.constants import get_pip_packages, get_post_install, get_pre_install, get_install, get_test_cmd
...
def setup_repo(container, build_path, workdir, repo_info, quick_mode=False):
    ...
    if dependencies_cmd != "":
        ...
        if "ERROR" in result_str:
            logger.error(
                f"Failed to install dependencies for {repo_info['repo_name']}"
            )
            return False
...
def evaluate_repo(repo_info, repos_dir):
    """
    评估repo的函数
    :param repo_info: repo信息
    :param repos_dir: repo存储路径
    :return: report 这个repo的评估结果
    """
    repo_url = repo_info["url"]
    repo_dir = os.path.join(repos_dir, repo_info["repo_name"])
    # 克隆repo
    ...
    try:
        image_tag = f"lpbench.valid.{repo_info['repo_name']}"  # 替换冒号以避免路径问题
        ...
        while retry_count < 3:
            ...
            if container is None:
                logger.error(f"Failed to build container for {repo_info['repo_name']}, retry {retry_count + 1}")
                retry_count += 1
                continue
            break
        ...
        # 设置repo环境
        success = setup_repo(container, build_path, workdir, repo_info, quick_mode=False)
        if not success:
        ...
        # 在container中运行测试
        test_cmd = f"""pytest --timeout={SINGLE_TEST_TIMEOUT} -rA --cov --cov-context=test -vv --cov-config=".coveragerc_tmp" --cov-report=json:coverage_report.json --json-report --json-report-file=pytest_report.json"""
        if get_test_cmd(repo_info["repo_name"]) is not None:
            test_cmd = get_test_cmd(repo_info["repo_name"])
            
        test_command = (
            f"""bash -c 'COVERAGERC_CONTENT="[run]\nbranch = true\nsource = .\nomit =\n    tests/*\n    test/*\n    */site-packages/*\n\n[json]\nshow_contexts = true\n"; echo "$COVERAGERC_CONTENT" > ".coveragerc_tmp"; {test_cmd}'"""
        )
        ...
        exec_result = container.exec_run(f"rm -rf /{repo_info['repo_name']}", workdir=workdir)
        result_str = exec_result.output.decode("utf-8")
        logger.success(f"result_str: {result_str}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Name of the repos to evaluate",
    )
    parser.add_argument(
        "--repos_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="Path to save harness reports",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    report_list = []
    # 过滤需要评估的项目
    items_to_eval = dataset
    if args.repos_name is not None:
        items_to_eval = [item for item in dataset if item["repo_name"] in args.repos_name]
    ...
    if args.workers > 1:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            future_to_item = {
                executor.submit(evaluate_repo, item, repos_dir): item
        ...
        for item in tqdm(items_to_eval, desc="Evaluating Repos"):
            try:
                result = evaluate_repo(item, repos_dir)
                if result is not None:
                    report_dict = result.to_dict()
                    item["tests"] = report_dict
                    report_list.append(item)
